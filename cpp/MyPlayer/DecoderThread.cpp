...
void VideoDecoderThread::releaseMediaInfo()
{
	...
	m_decoderContext->destroy_decoder_resource();
...
{	
	...
	while(!m_quit)
	{
		...
		if(frameFinished)
		{
			fprintf(stderr, "m_decoderContext->updateframe(m_frame), num = %ld\n", ++num);
			m_decoderContext->updateframe(m_frame);
		}
		long long end = Dayus();
		fprintf(stderr, "decode used: %lld\n", end - begin);
		if(m_decodePkt != NULL)
		{
			av_free_packet(m_decodePkt);
			av_freep(&m_decodePkt);
	...
	//	fprintf(stderr, "DecoderThread sleepTime %f\n", sleepTime);
...
/*	if(g_deviceContext == NULL)
	{
		g_deviceContext = new VdpauContext();
		g_deviceContext->init();
	}*/
	VideoPlayer::m_vdpauContext->init();
	m_decoderContext = new VdpauDecoderContext(m_vdpauContext);
//	m_dispContext = new VdpauDisplayContext(m_glDrawable, g_deviceContext);
	m_dispContext = new VdpauDisplayContext(m_vdpauContext);
	m_decoderContext->setDisplayHandle(m_dispContext);
	m_demux = new DemuxThread();
	m_videoDecoder = new VideoDecoderThread;
	m_videoDecoder->setVdpauContext(m_vdpauContext, m_decoderContext, m_dispContext);
	m_demux->setVideoDecoder(m_videoDecoder);
}

VdpauContext *VideoPlayer::m_vdpauContext = new VdpauContext();
...
{
	m_videoDecoder->releaseMediaInfo(); /*Important: release this first */
	m_demux->releaseVideoInfo();
}
